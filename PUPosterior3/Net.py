...
class PUPosterior3(NetWithLoss):
    ...
    def gradients(self, data, BS):
        ...
        a0 = np.mean(pmaxed0)
        if max_p < 0.95:
            self.alpha = np.mean(pmaxed1)*0.95
            print('alpha from 0.95 maxed posterior: ', self.alpha)
        ...
        if res.pvalue < 0.25:
            ...
            print('alpha if test not satisfied: ', self.alpha)
            useCVMLoss = 1
        else:
            #self.alpha = self.alpha + 0.01
            #print('alpha if test satisfied: ', self.alpha)
            useCVMLoss = 0
