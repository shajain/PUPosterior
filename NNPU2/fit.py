...
class PosteriorFitting:
    ...
    @classmethod
    def fromDataPU(cls, dataPU, alpha):
        x = dataPU.x
        nDims = x.shape[1]
        fitting = PosteriorFitting(nDims, alpha, debug=True)
        fitting.fit(dataPU)
    ...
    def fromDG(cls, dg, n, n1):
        ...
        sp.show()
        fitting = PosteriorFitting(nDims, dg.alpha, debug=True)
