...
class Debug:
    ...
    def plotHistLossHistoryPU(self):
        if not hasattr(self, 'histLosses'):
            self.histLosses={'train': [], 'val':[]}
        self.histLosses['train'].append(self.trainer.netPU.histLoss(self.data_tr))
        self.histLosses['val'].append(self.trainer.netPU.histLoss(self.data_val))
        sp.sortedplot(self.histLosses['train'], label='Tr', ax=self.axs[0, 1])
        sp.sortedplot(self.histLosses['val'], label='Val', ax=self.axs[0, 1])
        self.axs[0, 1].set_title('Histogram Loss')
    ...
    def plotDiscriminatorHistogram(self):
        x = self.data_val['x']
        x1 = self.data_val['x1']
        ...
        histProbs = puNet.histProbs(disc, puNet.posterior(x))
        histProbs1 = puNet.histProbs(disc1)
        bins = puNet.bins
        nBins = puNet.nBins
        pos = np.arange(nBins)
        barWidth = 0.2
        #pdb.set_trace()
        self.axs[1, 0].bar(pos - barWidth, histProbs1, color='r', width=barWidth, label='+')
        self.axs[1, 0].bar(pos + barWidth, histProbs, color='b', width=barWidth, label='+ul')
        posStr = ['l']
        posStr.extend([('{n:.2f}'.format(n=(bins[i]+bins[i+1])/2))[1:] for i in np.arange(nBins-2)])
        posStr.extend(['r'])
        #pdb.set_trace()
        #sp.xticks([ps for ps in pos], posStr)
        self.axs[1, 0].set_xticks(pos)
        self.axs[1, 0].set_xticklabels(posStr)
        self.axs[1, 0].set_title('Histogram')
    ...
    def afterPUUpdate(self):
        ...
        self.plotHistLossHistoryPU()
        self.plotLossPU()
        self.plotDiscLoss()
        self.plotDiscriminatorHistogram()
        if self.hasY:
            self.plotAUCs()
        self.displayPlots()
        #sp.show()
